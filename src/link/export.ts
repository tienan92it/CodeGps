...
export async function exportProjectConcepts(root: string): Promise<ExportStats> {
  ...
  try {
    ...
    const rows = know.prepare(`
      SELECT id, name, summary, domain, embedding
      FROM concepts
    `).all() as Array<{ id: string; name: string; summary: string | null; domain: string | null; embedding: Buffer | null }>;

    let withEmb = 0;
    const insert = gdb.prepare(`
      INSERT INTO concepts_global (id, project_id, local_concept_id, name, summary, domain, embedding, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
